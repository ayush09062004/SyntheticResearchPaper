...
FIXES vs original:
- _inject_fabricated_table: robust table extraction, validation, and fallback.
- No more broken or distorted tables.
- All prompts use textual citations (no \\cite) to avoid ?? placeholders.
- Improved table prompt to request a COMPLETE \\begin{table}...\\end{table}.
- Added re import for regex operations.
- Fixed indentation and record call in _inject_fabricated_table.
"""
...
_HALLUCINATION_SYSTEM = (
    ...
    "5. If the prompt asks for a paragraph, output plain LaTeX text.\n"
...
class HallucinationEngine:
    ...
    def _inject_fabricated_table(self, content: str, location: str) -> str:
        ...
        # Clean and extract the table environment
        payload = payload.strip()

        # If LLM returned a full \begin{table}...\end{table}, keep it as is
        if payload.startswith("\\begin{table}"):
            table_env = payload
        else:
            # Try to extract a tabular environment if present
            ...
            if match:
                ...
                tabular_body = (
                    ...
                    "\\midrule\n"
                    "Baseline A & 72.3 & 68.1 & 70.2 \\\\\n"
                    "Baseline B & 75.8 & 71.4 & 73.9 \\\\\n"
                    "Ours & 99.1 & 98.7 & 98.9 \\\\\n"
                    "\\bottomrule\n"
                    "\\end{tabular}"
                )
